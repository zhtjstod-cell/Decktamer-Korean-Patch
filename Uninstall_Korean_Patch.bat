...
"%PATCH_EXE%" --uninstall --game "%~1" --no-pause
goto finish

:run_exe_auto
"%PATCH_EXE%" --uninstall --no-pause
goto finish

:script_found
if "%~1"=="" goto run_auto
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%PATCH_SCRIPT%" -Mode Uninstall -GamePath "%~1"
goto finish

:run_auto
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%PATCH_SCRIPT%" -Mode Uninstall
